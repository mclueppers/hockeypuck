...
It is RECOMMENDED to keep the default POSTGRES_VERSION generated by `./mksite.bash` unless there is a specific need to upgrade.
Note that care must be taken when upgrading to PostgreSQL v18 and above, as it uses different mount points for data volumes (see below).
...
During this time, the hockeypuck service will be unavailable.

To upgrade the PostgreSQL container to v18, first refresh the `.env` file:

```
./mksite.bash
```

Then make sure that the following lines are uncommented in `.env`, and any conflicting `POSTGRES_VERSION` is removed:
