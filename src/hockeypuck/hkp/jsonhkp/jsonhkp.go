...
type Trust struct {
	...
	TrustTypeValue []byte  `json:"trustTypeValue"`
}
...
func NewTrust(from *openpgp.Trust) *Trust {
	...
	if typeNotation != nil {
		...
		to.TrustTypeValue = typeNotation.Value
	}
...
func (pk *PrimaryKey) packets() []*Packet {
	packets := pk.PublicKey.packets() // also includes Trusts and Signatures
	for _, u := range pk.UserIDs {
		packets = append(packets, u.packets()...)
	}
	for _, sk := range pk.SubKeys {
		packets = append(packets, sk.packets()...)
	}
	return packets
}
